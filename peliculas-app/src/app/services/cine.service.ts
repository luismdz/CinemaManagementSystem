...
export class CineService {
  ...
  obtenerTodosPorPagina(pageNumber: number = 1, pageSize: number = 50): Observable<CineDTO[]> {

    pageNumber = (pageNumber < 1) ? 1: pageNumber;
    pageSize = (pageSize < 5) ? 5: pageSize;
    
    return this.http.get<CineDTO[]>(`${this.apiUrl}?pageNumber=${pageNumber}&pageSize=${pageSize}`).pipe(
      map(resp => {
