...
export class GenerosEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.params.subscribe((params: any) => {
      ...
      if (this.editMode) {
        ...
        this.generoSvc.obtenerGeneroPorId(this.id).subscribe(
          (data) => {
            this.genero = data;
  ...
  guardarCambios() {
    ...
    if (this.editMode) {
      this.generoSvc.actualizarGenero(this.id, this.genero).subscribe(
        () => {
          this.router.navigateByUrl('/generos');
        },
        (err) => console.log(err)
      ...
      this.generoSvc.crearGenero(this.genero).subscribe(
        () => {
          this.router.navigateByUrl('/generos');
        },
        (err) => console.log(err)
